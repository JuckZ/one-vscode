...
export function getSortFn(pjson: any, fileName: string): sortPackageJsonExports {
  ...
  if (res?.packageJson)
    return getSortFn(res.packageJson, res.path)
  ...
  try {
    if (hasSortPackageJsonPkg(pjson)) {
      ...
      if (typeof sortPackageJson === 'function')
        return sortPackageJson
      if (sortPackageJson?.default && typeof sortPackageJson === 'function')
        return sortPackageJson.default
...
function hasSortPackageJsonPkg(json: any) {
  return (
    (json.dependencies && json.dependencies['sort-package-json']) || (json.devDependencies && json.devDependencies['sort-package-json'])
  )
}
